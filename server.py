...
LLAMA_API_URL: Optional[str] = os.getenv("LLAMA_API_URL", "http://localhost:11434/api/generate")
...
class PandaMCP(FastMCP):
    ...
    async def _call_anthropic(self, prompt: str) -> str:
        """
        Calls the Anthropic API to get a response for the given prompt.
    ...
    async def _call_llama(self, prompt: str) -> str:
        """
        Calls the LLaMA API (via a local server) to get a response for the given prompt.
    ...
    async def _call_gemini(self, prompt: str) -> str:
        """
        Calls the Google Gemini API to get a response for the given prompt.
    ...
    async def rag_query(self, question: str, model: str) -> str:
        """
        Performs a RAG query: retrieves context, then calls the specified LLM.
        ...
        if model == "anthropic":
            ...
            raise ValueError(f"Unsupported model '{model}'. Ensure this is the final else.")
...
async def rag_ask(request: QuestionRequest) -> Dict[str, str]:
    """
    Handles POST requests to the `/rag_ask` endpoint.
    ...
    Args:
        request (QuestionRequest): The request body, conforming to the
                                   QuestionRequest model, containing the
                                   user's question and the desired LLM model.
